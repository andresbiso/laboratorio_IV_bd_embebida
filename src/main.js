...
app.get('/', function(req,res){
  ...
  <p>You are going to perform CRUD operations<br>
  [CREATE] Please enter 'http://localhost:3000/add/(id number)/(name)' to add new employee to the database<br>
  [READ] 'http://localhost:3000/view/(id number)' to view an employee<br>
  [UPDATE] 'http://localhost:3000/update/(id number)/(new name)' to update an employee<br>
  [DELETE] 'http://localhost:3000/del/(id number)' to delete an employee<br>
  Before closing this window, kindly enter 'http://localhost:3000/close' to close the database connection </p>`);
});
